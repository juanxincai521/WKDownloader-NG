...
func RenameTXT() error {
	...
	for _, file := range files {
		...
		if remove {
			...
			err = os.Remove(data.DataPath + "/txt/" + file.Name())package rename
	...
	for _, file := range files {
		...
		for _, book := range data.NewData.Books {
			if bookNo == book.BookNo {
				...
				newBookName := strconv.Itoa(bookNo) + "_" + utils.CleanFileName(unquoteFileName) + ".txt"
...
func packCBZ(chapterNo int, volumeName, chapterName, bookDataPath, chapterTempPath string) error {
	...
	seelog.Info("归档图片成功："+ bookDataPath + "/" + cbzName)
	return nil
}
